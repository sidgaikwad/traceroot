...
>(
  (
    ...
    const [tokenCount, setTokenCount] = React.useState(0); // 🔹 new state
    ...
    const handleInput: React.FormEventHandler<HTMLTextAreaElement> = async (
      e
    ) => {
      adjustHeight();
      onInput?.(e);
      ...
      // 🔹 if input is cleared, reset token count immediately
      if (!text.trim()) {
        setTokenCount(0);
        return;
      }
...
console.log('PromptInputModelSelect rendered');
